...
export default function NavLinks({ links }: { links: NavLink[] }) {
  ...
  return (
    <>
      ...
      <nav className="sm:hidden fixed bottom-0 left-0 right-0 z-50 bg-gray-900 border-t border-gray-800 flex safe-area-inset-bottom">
        {links.map(link => {
          const isActive = pathname === link.href
          const { emoji, text } = parseLabel(link.label)
          // Shorten labels that are too long for bottom nav
          const shortText = text === 'Standings' ? 'Table' : text === 'My Team' ? 'Team' : text
          return (
            <Link
              key={link.href}
              href={link.href}
              className={`flex-1 flex flex-col items-center justify-center py-2.5 gap-0.5 min-w-0 transition-colors ${
                isActive ? 'text-yellow-400' : 'text-gray-500 active:text-gray-300'
              }`}
            >
              <span className="text-[22px] leading-none">{emoji}</span>
              <span className={`text-[10px] leading-none truncate w-full text-center font-medium ${
