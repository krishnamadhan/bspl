...
- Exactly 5 overs assigned in bowling order
- Max 2 overs per bowler
- No consecutive overs by the same bowler
- Minimum 4 distinct bowlers used
...
```
1. Insert innings rows (throws if fails → match stays lineup_open for retry)
2. Insert ball_log
3. Update match → completed
4. Upsert stamina
5. Upsert player_stats
6. Upsert points table
```
...
Currently uses a simplified formula:
```
NRR = (runs_for - runs_against) / (5 × matches_played)
```
The denominator `5 × played` assumes 5 overs per match. Actual overs bowled are not tracked per match for NRR purposes. This is a known inaccuracy (flagged for future fix).
...
1. **NRR denominator**: Hardcoded `5 × played` — should use actual overs bowled per match
2. **Admin lineup visibility**: Browser client can't see real-player lineups due to RLS. Fixed with `/api/admin/lineup-counts` service-role endpoint. If still broken, check Supabase RLS policy on `bspl_lineups`
3. **Concurrent simulations**: No locking mechanism — don't click Run twice simultaneously
4. **Run Full Season**: Only works for `in_progress` status, not `playoffs`
5. **T5 format**: Only 5-over matches implemented in the engine
6. **Exclusive draft**: Not implemented — same player can be on multiple bot teams
7. **Real-time updates**: No WebSocket/Supabase realtime — uses polling (MatchStatusPoller every 5s, admin page every 20s)
